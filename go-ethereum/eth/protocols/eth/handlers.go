...
// --- THESIS: GLOBAL PIGEON HTTP CLIENT ---
var pigeonClient = &http.Client{
	Timeout: 2 * time.Second,
	Transport: &http.Transport{
		MaxIdleConns:        50,
		MaxIdleConnsPerHost: 50,
		MaxConnsPerHost:     50, // THIS IS THE MAGIC BULLET: Limits open sockets to 50
	},
}
...
func handleNewPooledTransactionHashes(backend Backend, msg Decoder, peer *Peer) error {
	...
	for _, hash := range ann.Hashes {
		...
		// --- PIGEON NODE: ASYNC HTTP SENSOR START ---
		go func(txHash string, remoteIP string) {
			// Pull dynamic identity from Docker
			nodeID := os.Getenv("PIGEON_ID")
			if nodeID == "" {
				nodeID = "rogue-pigeon"
			}
			...
			// Fire payload using the throttled global client
			resp, err := pigeonClient.Post("http://127.0.0.1:8000/ingest", "application/json", bytes.NewBuffer(jsonData))
			
			// Safely discard the body and close the connection
			if err == nil {
				io.Copy(io.Discard, resp.Body)
				resp.Body.Close()
			}
		}(hash.Hex(), peer.Peer.RemoteAddr().String())
		// --- PIGEON NODE: ASYNC HTTP SENSOR END ---
	}
	return backend.Handle(peer, ann)
}
...
func handleTransactions(backend Backend, msg Decoder, peer *Peer) error {
	...
	for i, tx := range txs {
		...
		// --- PIGEON NODE: NETWORK INTERCEPTION LAYER START ---
		log.Info("🚨 [THESIS CAPTURE] PIGEON NODE SENSOR TRIGGERED", 
			"TxHash", tx.Hash().Hex(), 
			"Source_IP", peer.Peer.RemoteAddr().String())
		// --- PIGEON NODE: NETWORK INTERCEPTION LAYER END ---
	}
